...
void MCAL_SPI_SENDDATA	(SPI_typeDef *SPIx, uint16_t *pTxBuffer,enum PollingMechism PollingEn )
{
	...
	if (Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_2LINES || Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_1LINES_TXONLY)
	{
...
void MCAL_SPI_RECIEVEDATA	(SPI_typeDef *SPIx, uint16_t *pTxBuffer ,enum PollingMechism PollingEn )
{
	...
	if (Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_2LINES || Global_SPI_Config[index]->Communication_Mode == SPI_DIRECTION_1LINES_RXONLY)
	{
		if(PollingEn == SPI_Pollingenable)
...
//uint8_t APP_IS_VALID(uint8_t ID , uint8_t* arr)
//{
//	uint16_t is_valid ='0';
//
//	for(int i=0;i<3;i++)
...
//	*pTxBuffer = 0 ;
//	//APP
//	*pTxBuffer = (uint16_t)APP_IS_VALID(*pTxBuffer, arr);
//
//	//Send
//	if(PollingEn == SPI_Pollingenable)
//		while(!(SPIx)->SPI_SR & SPI_SR_TXE);
